...
### 4.1. Importar un conjunto de datos
...
En general, la función read.delim() detecta bien el tipo de separador que se ha empleado para delimitar las columnas. También podemos emplear la función read.table(), aunque en esta sí tenemos que indicarle cuál es el separador.
```{r}
...
### 4.3. Conocer la estructura de un conjunto de datos
...
### 4.4 Reordenar un conjunto de datos

### 4.5 Añadir filas y columnas

```{r}
rbind()
cbind()
df$new_column<-df$dias*2
```
...
* Podéis seguir trabajando sobre el conjunto de datos "tmb_mskcc_2018_clinical_data.tsv".  
